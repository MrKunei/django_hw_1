...
class AdListView(ListView):
    """Возвращает список объявлений с пагинацией(1 объявление -> 1 страница)"""
    model = Ad
...
class AdCreateView(CreateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # ad.author = get_object_or_404(User, pk=ads_data['author_id'])
        # ad.category = get_object_or_404(Category, pk=ads_data['category_id'])
...
class AdImageView(UpdateView):
    """Загрузка картинки к объявлению"""
    model = Ad
    fields = ['name', 'image']
...
class AdDetailView(DetailView):
    """Возвращает информацию по одному объявлению"""
    model = Ad
...
class UserAdDetailView(View):
    def get(self, request):
        user_qs = User.objects.annotate(total_ads=Count('ad', filter=Q(ad__is_published=True)))
